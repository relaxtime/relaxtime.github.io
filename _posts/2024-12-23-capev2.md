...
Смотрим статус dhcp-сервера `systemctl status isc-dhcp-server`
![](/images/capev2/Pasted%20image%2020241222234650.png)

Конфигурируем DHCP-сервер `sudo nano /etc/dhcp/dhcpd.conf`
...
Перезапускаем DHCP-сервер `sudo /etc/init.d/isc-dhcp-server restart`
![](/images/capev2/Pasted%20image%2020241222235146.png)

lease no subnet означает, что DHCP-сервер пока никому не выдал ip-адреса.

Посмотреть кому выданы адреса `sudo cat /var/lib/dhcp/dhcpd.leases`
Стартовать DHCP-сервер `systemctl start isc-dhcp-server`
Добавить DHCP-сервер в автозагрузку `systemctl enable isc-dhcp-server`
