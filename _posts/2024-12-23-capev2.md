...
После завершения на машине для анализа запустится клиент FOG, а на сервере FOG-сервере появится новая задача, которая отвечает за перезапуск машины и развертывание на ней образа.
