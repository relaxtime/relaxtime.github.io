...
DHCP-сервер VirtualBox в Типе подключения адаптера - NAT присвоит FOG-серверу IP-адрес 10.0.2.15. Этот адрес мы будем использовать далее в NatNetwork.
...
В менеджере сетей  VirtualBox во вкладке Сети NAT выбираем NatNetwork и отключаем DHCP-сервер, т.к. DHCP-сервер будет работать на FOG-сервере. Выход в сеть Интернет отсутствует.
...
Для аутентификации на FOG-сервере `http://10.0.2.15/fog/management/` используем учетную запись fog / password
...
В FOG-сервер успешно загрузился образ
...
После завершения анализа на FOG-сервере появится новая задача, которая отвечает за перезапуск машины для анализа и развертывания на ней записанного образа.
