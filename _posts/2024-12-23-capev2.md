...
Использование CAPE Sandbox и FOG для анализа вредоносного ПО
...
После этого можно сделать клон машины (с заменой mac-адреса) для установки на нее CAPE Sandbox.
...
**3. Устанавливаем CAPE Sandbox на виртуальную машину Ubuntu Desktop**
...
**4. Используя CAPE Sandbox запускаем ВПО на виртуальной машине для анализа, получаем отчет и возвращаем первоначальное состояние машине для анализа, восстановив образ с FOG-сервера**
...
Также на виртуальной машине запустится клиент FOG, а на FOG-сервере появится новая задача, которая отвечает за перезапуск машины и развертывание на ней образа.


По неизвестной мне причине клиент FOG на моей виртуальной машине для анализа не стартует, поэтому в меню FOG-сервера приходится выбирать deploy вручную
...
**4. Подключаем к хосту физическую машину для анализа**
